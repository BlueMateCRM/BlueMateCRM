...
const getZip = async (e: any) => {
  ...
  vehicle.delivery_city = res.data.data?.[0]?.id;
};
const selectZip = (item: any) => {
  vehicle.delivery_city = item?.name;
  vehicle.delivery_state = item?.state?.name;
  vehicle.delivery_zip = item?.zip;
...
const openEditForm = () => {
  ...
  vehicle.delivery_city = props.data?.name;
  vehicle.delivery_state = props.data?.state?.name;
  vehicle.delivery_zip = props.data?.zip;
};

const submitData = () => {
  props.editFunction({ destination: city_id.value });
  ...
  <form @submit.prevent="submitData">
    ...
    >
      <div class="w-1/3 text-nowrap overflow-hidden">
        ...
        <span class="text-darkBlue font-medium">Destination</span>
      </div>
      ...
      >
        <template #title>
          <div class="flex items-center">
            ...
            <span class="text-darkBlue font-medium capitalize"> delivery </span>
          </div>
        </template>
        <template #btns>
          <button
        ...
        <template #body>
          ...
          <div class="flex items-center mb-[10px]">
            <label for="delivery_p_city" class="w-[35%] text-textBlack">
              Delivery city
            </label>
            <input
              id="delivery_p_city"
          ...
          <!-- 2 -->
          <div class="flex items-center mb-[10px]">
            <label for="delivery_p_state" class="w-[35%] text-textBlack">
              Delivery state
            </label>
            <input
              id="delivery_p_state"
          ...
          <!-- 3 -->
          <div class="flex items-center">
            <label for="delivery_vehicle_zip" class="w-[35%] text-textBlack">
              Delivery zip
            </label>
            <div class="relative w-[65%]">
              <input
                id="delivery_vehicle_zip"
